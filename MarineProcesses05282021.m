function [Hstar,P3,MeanBeachWidth,ESL,WSL,MESL,MWSL,OL,SLRyrs,MigCnt,MigAccel]=MarineProcesses05172021(Hstar,delta,L,P1,P2,P3,P4,BchMax,OW,t,SLRyrs,PCmp,IslandArea,MigCnt,ScaleFactor,MigYr,Ma,MigAccel,MasterMax,SLRvec,BruunParam)
alpha=1;
AoR=pi/6;
t;
T=t/26;
...
nn=5;   %number of rows above and below current row to check plant density when calculating migration
    ...
    if MigChk==1
        for i=1:n1
            if i<nn
              ...
              Plx(i)=min(sum(PlChkArea(PlChkArea>=0))/15,1);
...
%                 end
                ...
                Plx(i)=min(sum(PlChkArea(PlChkArea>=0))/30,1);
              %  Plx(i)=mean(PlChkArea(PlChkArea>=0));
               if sum(dH(i,:)>=0)==0
                    MigCnt(i)=MigCnt(i)+Mig;
                else
